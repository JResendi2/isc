	...
	************************************************************/	
	if ($datosAEliminar != null) { 
		...
		for ($i=0; $i < count($datosAEliminar); $i++) { // count() --> obtiene la longitud del arreglo
			...
			// SEGUNDO: Eliminar los temas de la materia y sus relaciones con otas materisa
			// Esto se hace automaticamente con la eliminacion en cascada
			$sql = "delete from mat_sem_prof where idmateria = '". $id. "'";
