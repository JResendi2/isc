	...
	************************************************************/	
	if ($datosAEliminar != null) { 
		...
		for ($i=0; $i < count($datosAEliminar); $i++) { // count() --> obtiene la longitud del arreglo
			...
			// Formular la consuLta para eliminar el registro de "contenidos" donde el id sea igual a la variable $id
			...
			$sql = "delete from imagenes where ids = '". $id. "' and tipo='contenido'";
			mysqli_query($conexion, $sql); // Ejecutar la consulta

			// Formular otra consulta para eliminar el registro de "archivos" relacionado a la variable $id
